...
class LanguageUnderstandingResult(BaseModel):
    ...
    needs_memory: bool = Field(default=False, description="Si l'intention nécessite un accès à la mémoire ou à l'historique")
    needs_tools: bool = Field(default=False, description="Si l'intention nécessite l'usage d'outils")
    risks_security: str = Field(default="normal", description="Évaluation du risque de sécurité de l'action (normal, sensitive, critical)")
